...
src/demo/                fixtures только явного демонстрационного режима
src/features/            доменные модули и общая логика синхронизации
...
Supabase — источник истины. Web и Flutter должны читать и изменять одни и те же таблицы и RPC. Для серверных операций UI обновляет доменное состояние после подтверждения записи. Offline-очередь повторяет только временные ошибки и сохраняет стабильный ключ идемпотентности.
